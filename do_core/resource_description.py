    ...
    def __readEndpointsAndVlans(self):
        ...
        for interface in self.__dict["netgroup-domain:informations"]["netgroup-network-manager:informations"]["openconfig-interfaces:interfaces"]["openconfig-interfaces:interface"]:
            ...
            if interface_vlan["interface-mode"] == "TRUNK":
                ...
                ep['free_vlans'] = interface_vlan["trunk-vlans"] #reference
                ''' 
                    ep['free_vlans'] has the reference to list object "trunk-vlans" inside self.__dict.
                    Every modification on ep['free_vlans'] will affect the list object "trunk-vlans".
                '''
                if len(ep['free_vlans'])==0:
                    update_needed=True
                    ep['free_vlans'].clear()
                    ep['free_vlans'].extend(self.__config_vlan_available_ids)
                
                # Create an array of vlan id ranges
                #ep['free_vlans'].clear()
                #ep['free_vlans'].append = self.__set_available_vlan_ids_array(ep['free_vlans'])
        ...
        for endpoint_name in self.__endpoints:
            ...
            for q in query:
                ...
                if q.vlan_in is not None:
                    #self.__endpoints[endpoint_name]['free_vlans'].append(int(q.vlan_in))
